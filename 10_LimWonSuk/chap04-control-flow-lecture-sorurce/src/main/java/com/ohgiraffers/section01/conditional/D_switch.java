...
/* switch(식)
    ...
    break;
    ...
    default : 모든 case가 일치하지 않을 때 수행될 코드
    ...
    * *//*
    ...
    public void testSimpleSwitchStatement() {
        ...
        switch (op)
        {
            ...
            case '-' : result = first - second; break;
            case 'X' : System.out.println("X 선택됨");// break가 없다면 다음 case로 이동됨
            case 'x' : System.out.println("x 선택됨");// break가 없다면 다음 case로 이동됨
            case '*' : result = first * second; break;
            case '/' : result = first / second; break;
            case '%' : result = first % second; break;
            default :
                ...
                return; // 메서드를 끝내고 돌아감, return 이없으면 잘못되어도 답이 나오게됨
                    // early return 메서드가 끝나가전에 중간에 리턴시킴
        }
...
/* Java 14 버전에 추가된 case 작성 방식 *//*
    ...
    public void improveSimpleSwitchStatement() {
        ...
        int result = switch (op) // 연산 결과 저장용 변수
        {
